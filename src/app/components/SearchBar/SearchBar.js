...
function SearchBar({ clicked, setClicked }) {
    ...
    const changeHandler = async (e) => {
        ...
        let employees = []
        if (e.target.value.length >= 2 && employees.length === 0) {
            employees = await fetchUsers(e.target.value)
            setEmployees([...employees])
        }
        setFilteredEmployees([...employees].filter((emp) => {
            return emp.name.toLowerCase().includes(e.target.value.toLowerCase()) || emp.workTitle.includes(e.target.value.toLowerCase())
        }))
    }
                    ...
                    <div className='itemsList'>
                        {
                            filteredEmployees.map((employee) => {
                                return (
                                    <Item key={employee._id} employee={employee} search={search} />
                                )
                            })
                        }
                    </div>
